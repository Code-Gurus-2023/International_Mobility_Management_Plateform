...
public class CandidacyRestController {
    ...
    /*@GetMapping("/statut/{statutsCandidacy}")
    public ResponseEntity<List<Candidacy>> getCandidaturesByStatut(@PathVariable("statutsCandidacy")
                                                                       StatusCandidacy statutsCandidacy) {
        List<Candidacy> candidatures = candidacyService.getCandidaturesByStatut(statutsCandidacy);
        return new ResponseEntity<List<Candidacy>>(candidatures, HttpStatus.OK);
    }

    // Récupérer toutes les candidatures par statut différent de celui spécifié
    @GetMapping("/statut-not/{statutsCandidacy}")
    public ResponseEntity<List<Candidacy>> getCandidaturesByStatutNot(@PathVariable("statutsCandidacy")
                                                                          StatusCandidacy statutsCandidacy) {
        List<Candidacy> candidatures = candidacyService.getCandidaturesByStatutNot(statutsCandidacy);
        return new ResponseEntity<List<Candidacy>>(candidatures, HttpStatus.OK);
